...
export async function scrapeTopics(topicGroups: TopicGroup[]): Promise<ScrapedTopicGroup[]> {
  ...
  for (const group of topicGroups) {
    ...
    for (const story of group.stories) {
        ...
        // Fix 2: Google News URL detection - skip fetch
        if (story.url.includes('news.google.com')) {
          console.log(`[SCRAPER] Google News URL detected — using RSS summary directly: ${story.url}`);
          ...
          if (summaryWords.length >= 30) {
            ...
            scrapeSuccess = true;
            console.log(`[SCRAPER] Successfully used RSS summary for Google News (${summaryWords.length} words).`);
          } else {
            console.log(`[SCRAPER] RSS summary too short for Google News: ${summaryWords.length} words.`);
          }
        } else {
        ...
        if (!scrapeSuccess && !story.url.includes('news.google.com')) {
          const fullSummary = (story as any).fullSummary || '';
          const summaryWords = fullSummary.split(/\s+/).filter(w => w.length > 0);
      ...
      if (scrapedTexts.length >= minRequired) {
        break;
      }
    }
    ...
    if (scrapedTexts.length >= minRequired) {
      ...
      console.log(`[SCRAPER] Group ${group.topic_name} successfully scraped enough articles (${scrapedTexts.length}/${minRequired}).`);
    } else {
      console.log(`[SCRAPER] Group ${group.topic_name} failed to get enough scraped articles (got ${scrapedTexts.length}/${minRequired}). Skipping group.`);
    }
  }
