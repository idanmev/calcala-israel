...
async function loadHomepageArticles() {
  try {
    ...
    // Render hero
    if (heroArticle) {
      const title = escapeHtml(heroArticle.title);
      const subtitle = heroArticle.subtitle ? `<p class="text-lg text-gray-200">${escapeHtml(heroArticle.subtitle)}</p>` : '';
      const catName = escapeHtml(heroArticle.categories?.name || 'כללי');
      ...
      const heroContainer = document.querySelector('[data-hero-article-container]');
      if (heroContainer) {
    ...
    if (gridData.length > 0) {
      const gridHtml = gridData.map(article => {
          ...
          <a href="/article/${encodeURIComponent(article.slug)}" class="bg-white border border-gray-200 overflow-hidden hover:shadow-lg transition-shadow cursor-pointer block group rounded-lg">
            ...
            <div class="p-5">
              <div class="flex items-center gap-3 mb-2 text-sm relative z-10">
                <span class="text-red-600 font-bold bg-red-50 px-2 py-0.5 rounded">${escapeHtml(article.categories?.name || 'כללי')}</span>
                <span class="text-gray-500 font-medium">${formatHebrewDate(article.publish_date)}</span>
              </div>
              <h3 class="text-xl font-bold mb-2 group-hover:text-red-600 transition-colors line-clamp-2 leading-tight">${escapeHtml(article.title)}</h3>
              ${article.subtitle ? `<p class="text-gray-600 text-sm line-clamp-2">${escapeHtml(article.subtitle)}</p>` : ''}
            </div>
          </a>
        `;
      }).join('');

      const gridContainer = document.querySelector('[data-articles-grid-container]');
      if (gridContainer) {
